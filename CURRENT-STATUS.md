...
| `component-catalog` | 2026-08-30 | Make IDE surfaces and agent CLIs regular catalog components: a neutral `codium` interactive surface replacing the special-cased `codium_with_claude` path, then the Antigravity CLI as a default-selected, just-in-time-materialized agent component with per-checkout persistent state | active; track 1 (codium surface) and track 2 (Antigravity CLI, codex npm delivery, refusal-UX ruling, base-family matrix) integrated through PRs #50, #55, #56, #57; the 0.2.10 release walk is mid-flight — base pushed, repin and dogfood three-agent migration on the branch awaiting PR, owner smokes, tag, then 0.2.9 withdrawal | `component-catalog/antigravity-cli` (current working branch); `component-catalog/outbox` | [workstream status](engineering-docs/wip/2026-08-30-component-catalog/CURRENT-STATUS.md) |
