...
### When Init Refuses, And When It Only Discloses

Settled with the product owner on 2026-09-06, after taking stock of every
refusal the tool issues and who receives it. Three people stand on the
other side of a refusal, distinguished by phase rather than by identity:
the **project owner** at `init`, the only place resolution against the
matrix happens; a **collaborator** at `config`, `resolve`, and `run`, who
consumes a lock and meets only consent and drift; and the **matrix
maintainers**, who decide what counts as validated and are also the
heaviest owners, permanently running ahead of their own matrix.

Refusals fall on these grounds:

| Ground | Who meets it | Nature | Lever |
| --- | --- | --- | --- |
| Vocabulary and shape: unknown capability, zero or two surfaces, malformed files | owner | a fact about the request | fix the request |
| A base does not ship a required toolchain | owner | a fact about the world | none; the message says `--unverified` cannot help |
| A component or coupling not yet validated on the selected base | owner | a fact about the matrix's knowledge | run it as an experiment with `--unverified` |
| Consent declined: base image, vendor acquisition | anyone, per checkout | the user's decision | authorize later |
| A published base other than the pinned digest | anyone | reproducibility rule | select a local image instead |
| Drift: old recipe, already initialized, authored content never rewritten | anyone | protocol | `--regenerate`, the `config` family |

The third row is not a refusal on grounds and is not phrased as one. The
message is short: it names the exact elements the experiment would run —
component, version, base, never the matrix's internal vocabulary — and
offers the experiment. The lever is accepted by every command that can
print it. The lock generated by an experiment carries the same sentence in
its header and in `unverified-combinations`, so the owner or contributor
who commits it and the collaborator who clones it read one fact. For now
both are warned the same way; how the matrix learns from an adopter's
successful experiment, and how a matrix change is later gated on a claim
that every added combination ran, is with `project-management`
(intake 2026-09-06; see the D-0007 amendment of the same date).
